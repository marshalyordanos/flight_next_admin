...
import type { Role, RoleType, RolePermission } from '@/@types/role'
...
const RoleEditPage = () => {
    ...
    useEffect(() => {
        const fetch = async () => {
            ...
            setLoading(true)
            try {
                const res = await roleService.getOne(id)
                const r = res.data
                setRole(r)
                reset({ name: r.name, type: r.type as RoleType })
                const perms = Array.isArray(r.permissions)
                    ? (r.permissions as RolePermission[])
                    : []
                    ...
                    <FormItem label="Permissions">
                        <div className="space-y-2 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
                            ...
                            {SUBJECTS.filter((s) => s !== 'ALL').map(
                                (subject) => (
                                    <div key={subject} className="pl-6">
                                        <Checkbox
                                            checked={selectedPermissions.some(
                                                (p) => p.subject === subject,
                                            )}
